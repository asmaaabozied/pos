...
class BlogsController extends Controller
{
    ...
    {  
        ...
        if (!file_exists('frontend/images/blog')) {
            mkdir('frontend/images/blog', 0777, true);
        }
        ...
        if ($thumbnail) {
            ...
            $thumbnail->move('frontend/images/blog', $imageName);

            list($width, $height) = getimagesize('frontend/images/blog/'. $imageName);
            if ($width > $height) {
                Image::make('frontend/images/blog/'. $imageName)->fit(1000, 600)->save('frontend/images/blog/'. $imageName);
            } elseif($width > $height){
                Image::make('frontend/images/blog/'. $imageName)->fit(600, 600)->save('frontend/images/blog/'. $imageName);
            } else {
                Image::make('frontend/images/blog/'. $imageName)->fit(600, 1000)->save('frontend/images/blog/'. $imageName);
            }
        ...
        if ($og_image) {
            ...
            $og_image->move('frontend/images/blog', $imageName);
