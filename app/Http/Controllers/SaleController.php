...
class SaleController extends Controller
{
    ...
    public function saleData(Request $request)
    {
        ...
        if (!empty($sales)) {
            foreach ($sales as $key => $sale) {
                ...
                $nestedData['biller'] = $sale->biller->name ?? null;
                $nestedData['customer'] = $sale->customer->name ?? null . '<br>' . $sale->customer->phone_number . '<input type="hidden" class="deposit" value="' . ($sale->customer->deposit - $sale->customer->expense) . '" />' . '<input type="hidden" class="points" value="' . $sale->customer->points . '" />';
                ...
                $nestedData['sale'] = array('[ "' . date(config('date_format'), strtotime($sale->created_at->toDateString())) . '"', ' "' . $sale->reference_no . '"', ' "' . $sale_status . '"', ' "' . $sale->biller->name ?? null . '"', ' "' . $sale->biller->company_name ?? null . '"', ' "' . $sale->biller->email ?? null . '"', ' "' . $sale->biller->phone_number ?? null . '"', ' "' . $sale->biller->address ?? null . '"', ' "' . $sale->biller->city ?? null . '"', ' "' . $sale->customer->name ?? null . '"', ' "' . $sale->customer->phone_number ?? null . '"', ' "' . $sale->customer->address ?? null . '"', ' "' . $sale->customer->city ?? null . '"', ' "' . $sale->id . '"', ' "' . $sale->total_tax . '"', ' "' . $sale->total_discount . '"', ' "' . $sale->total_price . '"', ' "' . $sale->order_tax . '"', ' "' . $sale->order_tax_rate . '"', ' "' . $sale->order_discount . '"', ' "' . $sale->shipping_cost . '"', ' "' . $sale->grand_total . '"', ' "' . $sale->paid_amount . '"', ' "' . preg_replace('/[\n\r]/', "<br>", $sale->sale_note) . '"', ' "' . preg_replace('/[\n\r]/', "<br>", $sale->staff_note) . '"', ' "' . $sale->user->name ?? null . '"', ' "' . $sale->user->email ?? null . '"', ' "' . $sale->warehouse->name ?? null . '"', ' "' . $coupon_code . '"', ' "' . $sale->coupon_discount . '"', ' "' . $sale->document . '"', ' "' . $currency_code . '"', ' "' . $sale->exchange_rate . '"]'
                );
                $data[] = $nestedData;
            }
        }
        ...
        if ($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && $data['paid_amount'] > 0)) {
            foreach ($data['paid_by_id'] as $key => $value) {
                if ($data['paid_amount'][$key] > 0) {
                    ...
                    $lims_payment_data->cash_received = $data['paying_amount'][$key];
                    $lims_payment_data->currency = $data['currency'][$key];
    ...
    public function posSale($id = '')
    {
        ...
        if ($role->hasPermissionTo('sales-add')) {
            ...
            $currency_list = Currency::where('is_active', true)->get();
            // $ratecurrency = Currency::where('code', 'Euro')->first()->exchange_rate;
            $numberOfInvoice = Sale::count();
            $custom_fields = CustomField::where('belongs_to', 'sale')->get();
    ...
    public function currencySaleData(Request $request)
    {

        $rate = Currency::where('name', $request->currency)->first()->exchange_rate ?? 0;
    ...
    public function getPayment($id)
    {
        ...
        foreach ($lims_payment_list as $payment) {
            ...
            $paid_amount[] = round($payment->amount, 2);
            $cash_received[] = round($payment->cash_received, 2);
    ...
    public function todaySale()
    {
        ...
        $data['cash_payment_currency'] = Payment::where([
            ['paying_method', 'Cash']
        ])->select('currency', 'cash_received')->get()->groupBy('currency')->map(function ($item,$currency) {
            return ['currency'=>$currency,'cash_received'=>$item->sum('cash_received')];
        })->toArray();
        ...
        $data['credit_card_payment_currency'] = Payment::where([
            ['paying_method', 'Credit Card']
        ])->whereDate('created_at', date("Y-m-d"))->select('currency', 'cash_received')->get()->groupBy('currency')->map(function ($item,$currency) {
            return ['currency'=>$currency,'cash_received'=>$item->sum('cash_received')];
        })->toArray();
        ...
        $data['gift_card_payment_currency'] = Payment::where([
            ['paying_method', 'Gift Card']
        ])->whereDate('created_at', date("Y-m-d"))->select('currency', 'cash_received')->get()->groupBy('currency')->map(function ($item,$currency) {
            return ['currency'=>$currency,'cash_received'=>$item->sum('cash_received')];
        })->toArray();
        ...
        $data['deposit_payment_currency'] = Payment::where([
            ['paying_method', 'Deposit']
        ])->whereDate('created_at', date("Y-m-d"))->select('currency', 'cash_received')->get()->groupBy('currency')->map(function ($item,$currency) {
            return ['currency'=>$currency,'cash_received'=>$item->sum('cash_received')];
        })->toArray();
        ...
        $data['cheque_payment_currency'] = Payment::where([
            ['paying_method', 'Cheque']
        ])->whereDate('created_at', date("Y-m-d"))->select('currency', 'cash_received')->get()->groupBy('currency')->map(function ($item,$currency) {
            return ['currency'=>$currency,'cash_received'=>$item->sum('cash_received')];
        })->toArray();
        ...
        $data['paypal_payment_currency'] = Payment::where([
            ['paying_method', 'Paypal']
        ])->whereDate('created_at', date("Y-m-d"))->select('currency', 'cash_received')->get()->groupBy('currency')->map(function ($item,$currency) {
            return ['currency'=>$currency,'cash_received'=>$item->sum('cash_received')];
        })->toArray();
